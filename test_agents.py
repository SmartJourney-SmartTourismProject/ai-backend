...
async def test_agents():
    ...
    wa = WeatherAgent()
    result = await wa.execute(state)
    wkeys = list(state.weather.keys()) if state.weather else None
    print('  Weather: success=' + str(result.success) + ', weather_keys=' + str(wkeys))
    
    print('Testing DisasterAgent...')
    da = DisasterAgent()
    result = await da.execute(state)
    alert_count = len(state.disaster.get('alerts', [])) if state.disaster else 0
    print('  Disaster: success=' + str(result.success) + ', alerts=' + str(alert_count))
