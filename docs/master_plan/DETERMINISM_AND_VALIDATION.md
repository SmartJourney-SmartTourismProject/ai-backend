...
> **⚠ Verified 2026-09-02, rung 1 doesn't apply to our model.** A live call to
> `gemini-3.5-flash-lite` returns:
> `UserWarning: Model 'gemini-3.5-flash-lite' uses fixed sampling defaults; the sampling
> parameter(s) temperature, top_k, top_p will be ignored.`
> The API accepts the parameters without erroring, but this exact model silently ignores them —
> it always samples at its own fixed (undocumented) setting. **This is not a config bug and
> nothing needs fixing**; it just means rung 1 contributes nothing for us, and the real
> determinism budget is entirely rungs 2–5, especially rung 5 (deterministic Python ranking,
> §6–7) — which was already carrying the "very high" rating for good reason. Still set
> `temperature=0` in code: it's free, it's correct for any provider in the fallback chain that
> *does* honor it (Groq's `openai/gpt-oss-120b` does), and removing it would only lose that.

---

## 2. LLM configuration

**✅ Implemented and live-verified 2026-09-02** — `app/config/settings.py` and `app/core/llm.py`
carry exactly this. `get_llm("slots")` was smoke-tested with a real `ainvoke()` against Gemini and
returned correctly through the `RunnableWithFallbacks` wrapper.
...
#
# The two client classes' constructor field names genuinely differ (checked
# against their pydantic model_fields, not assumed): ChatGoogleGenerativeAI
# takes top_p/top_k/max_output_tokens/timeout; ChatGroq has NO top_p field
# at all and calls its equivalents model_name/max_tokens/request_timeout.
# A shared kwargs dict silently breaks one of the two - branch per provider.

_TOKEN_BUDGET = {"slots": 512, "orchestrator": 1024, "recommend": 2048, "plan": 3072, "respond": 512}
...
def _build(spec: str, purpose: Purpose) -> BaseChatModel:
    ...
    if provider == "gemini":
        return ChatGoogleGenerativeAI(
            ...
            temperature=settings.llm_temperature, top_p=settings.llm_top_p, top_k=settings.llm_top_k,
            max_output_tokens=max_tokens, timeout=settings.llm_timeout_s, max_retries=0,
        )
    if provider == "groq":
        return ChatGroq(
            model_name=model, groq_api_key=settings.groq_api_key,
            temperature=settings.llm_temperature, max_tokens=max_tokens,
            request_timeout=settings.llm_timeout_s, max_retries=0,
        )
    raise ValueError(f"unknown LLM provider in chain: '{provider}'")

@lru_cache(maxsize=16)
def get_llm(purpose: Purpose) -> BaseChatModel:
    """Primary + the rest of the provider chain as .with_fallbacks(), which
    preserves .bind_tools() and .with_structured_output() on what it returns -
...
Previously `ChatGoogleGenerativeAI(...)` was constructed in three places with three different
configurations (and `RecommendationAgent`/`PlanningAgent` didn't even pass the API key, relying on
`load_dotenv()` having populated `os.environ` — which worked, but only because `main.py` called it
...
### Model id — verified, plus one live finding

`gemini-3.5-flash-lite` confirmed callable on the project's key (`scripts/check_apis.py --llm` →
38 models available, this one among them). Real RPM/RPD ceiling: check
[aistudio.google.com/rate-limit](https://aistudio.google.com/rate-limit) — Google no longer
publishes it, and it can change.

**Also discovered live:** this exact model ignores `temperature`/`top_p`/`top_k` — see the callout
under §1 above. Not a bug, just means rung 1 of the determinism ladder is a no-op for our primary
provider; the plan already didn't depend on it being the main lever.
...
> **Data source, confirmed live 2026-09-02 (see `API_SETUP.md §4.1`):** `travel_listing.rating` is
> populated for **hotels only**, from Booking.com's `reviewScore`/`reviewCount` (already fetched
> alongside pricing, no extra call). Foursquare's `rating` field turned out to be Premium-only even
> on its free tier — not used. Restaurants/attractions have no free star-rating source, so `rate()`
> correctly falls to the 0.45 neutral prior for them via the `item.rating is None` branch below —
> this was already the right degradation path, it just applies to two whole categories, not a few
> stragglers. `wikidata_enrich` writes a separate `popularity_prior` (Wikipedia langlinkscount) for
> exactly this gap; wiring it into `rate()` as a secondary signal for the rating-less categories is
> a Phase 4 decision when `scoring.py` is actually built, not a change to this formula's contract.
